  ...
  transacoes.forEach((t) => {
    ...
    for (let i = atual; i <= final; i++) {
      ...
      const dataParcela = new Date(
        dataFaturaBase.getFullYear(),
        dataFaturaBase.getMonth() + mesesAAdicionar,
        1,
      );
    ...
    const buscarUsuario = async () => {
      ...
      try {
        ...
        const transacoesAchatadas =
          resposta.data.periodos?.flatMap((periodo) => {
            const mesAnoFatura =
              periodo.mesAno || `${periodo.mes}/${periodo.ano}`;
            return (periodo.transacoes || []).map((t) => ({
              ...t,
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 text-xs text-gray-500 mt-1 items-center">
                  ...
                  <span className="text-[10px] text-gray-400">
                    Comprado em {t.dataCompraOriginal}
                  </span>
                </div>
              </div>
              <div className="text-right ml-2 shrink-0">
                <p
                  className={`font-bold ${t.tipo === "debito" ? "text-red-500" : "text-green-500"}`}
                >
      ...
      <div className="flex flex-col h-full md:hidden">
        <div className="flex-1 overflow-y-auto">
          {abaAtiva === "transacoes" ? (
            ...
            <div className="bg-gray-200 min-h-full">{colunaGraficos}</div>
          )}
        </div>
